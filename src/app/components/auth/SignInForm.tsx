...
interface SignInFormProps {
  onToggle: () => void;
  onForgotPassword?: () => void; // Optional prop for handling password reset
}
...
export function SignInForm({ onToggle, onForgotPassword }: SignInFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle email logins disabled
      if (error.message?.includes('Email logins are disabled') || error.message?.includes('email_disabled')) {
        toast.error('Email/password login is currently disabled. Please contact the administrator.');
        return;
      }
      ...
      // Supabase specific error mapping or generic fallback
      ...
      <div className="px-8 pb-8">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 text-white font-medium py-2.5 px-4 rounded-xl hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-blue-600/30"
          >
            {loading ? (
              <>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button 
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-200"
          >
             {/* Simple Google G placeholder or use an SVG */}
